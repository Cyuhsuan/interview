# 前端產品與客戶指南

## 患者將使用的功能

前端將是一套支援桌面與行動裝置、僅使用英文的 Web 預約客服。患者可打字或使用麥克風；兩種方式都會使用相同的預約對話與後端規則。

介面只有一個主要任務：協助患者選擇支援的服務，並預約安全且可用的時段。它不得呈現為臨床診斷工具。

## 規劃中的患者流程

1. 選擇或描述服務 A–E。
2. 查看服務時間與可執行的專業人員類別。
3. 選擇日期。
4. 選擇可預約的專業人員與開始時間。
5. 提供患者姓名與接收行事曆邀請的 email。
6. 檢查服務、時間長度、專業人員、日期、時間、時區及 email。
7. 明確確認預約。
8. 取得預約參考 ID 及行事曆傳送狀態。

對話中顯示的可預約狀態僅為暫時結果。患者確認時，後端會再次檢查。若時段已被占用，介面必須說明狀態已改變並提供新的選項。

## 語音行為

第一版會在可用的瀏覽器上使用 speech recognition 與 speech synthesis。這可避免原始音訊經過應用程式後端，也可使產品不綁定特定語音供應商。

預期限制：

- 不同瀏覽器與作業系統的支援程度不同。
- 部分瀏覽器可能透過自己的雲端服務處理語音。
- 麥克風權限可能被拒絕或無法使用。
- 姓名、日期與 email 可能被錯誤辨識。

因此，語音只能是選用功能。執行任何不可逆操作前都必須顯示辨識後的文字；敏感欄位必須可編輯，且預約必須經過明確確認。

## 服務範圍文案

介面可以協助安排標準預約，但必須拒絕：

- 症狀評估或診斷。
- 緊急或急症照護判斷。
- 處方或治療建議。
- 治療價格、保險範圍或付款。
- 需要身分驗證的取消或改期。

上線前，診所必須提供日常聯絡電話、緊急情況文案、營業時間、隱私聲明及支援聯絡方式。

## App 外部的客戶設定

發布 Web 應用程式前，診所必須：

1. 核准服務名稱、時間、可執行人員、營業時間、假日、休息時段及提前預約規則。
2. 選擇診所擁有的 HTTPS domain，例如 `appointments.exampleclinic.com`。
3. 發布隱私聲明，說明預約資料及可能使用的瀏覽器語音服務。
4. 確認緊急情況文案及員工電話轉接方式。
5. 完成[後端指南](../backend/README.md)中的 Google Cloud 與 Microsoft Entra 設定。
6. 提供 Logo、診所名稱、聯絡資訊及符合無障礙標準的核准色彩。
7. 在接受真實資料前，使用合成患者資料與 sandbox 行事曆測試。

## 診所驗收清單

- A、B 可由三位專業人員執行。
- C、D、E 絕不顯示 Junior。
- 六小時服務若會超過關門時間，絕不提供該時段。
- Google 與 Outlook 中的忙碌活動會移除重疊時段。
- 結帳期間被占用的時段會改為提供新選項，不會產生重複預約。
- 確認後顯示正確服務、人員、時區與時間長度。
- 醫療、緊急情況、保險、價格及取消要求會得到核准的轉接文案。
- 在純文字、純鍵盤、螢幕閱讀器、行動裝置、麥克風遭拒及不支援語音的環境下都可完成預約。
- 瀏覽器 request、storage 或 production bundle 中不可看見行事曆或 AI credential。

## 規劃中的營運指引

若無法驗證可預約狀態，頁面應請患者稍後重試或聯絡診所，不得猜測。只有後端已產生參考 ID，且營運系統顯示行事曆同步成功或存在有效的 retry alert 時，員工才應將該筆預約視為已完成。
